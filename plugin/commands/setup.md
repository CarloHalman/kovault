...
service stood up alongside it. `/kovault:setup` only connects the plugin to that backend.
...
5. Verify: call the Kovault `lookup` tool with a trivial term (or `rows` on `settings`) and
   ...
   Report success and remind the user they can change any of this with `/kovault:settings`.
